...
class MyClass extends Object {
    ...
    // : 두 객체가 동등한지 비교
    // - 두 객체의 참조를 비교 (동일한 주소값을 가지는지 비교)
    // - 오버라이딩(재정의): 객체 내용 기반의 비교를 구현 가능
}

public class JavaLang {
    public static void main(String[] args) {
    MyClass myClass1 = new MyClass(1, "최명준");
//        System.out.println(myClass1.toString());
        System.out.println(myClass1.toString()); // MyClass(ID: 1 / NAME: 최명준)

        MyClass myClass2 = new MyClass(2, "정단영");
        ...
        System.out.println(Math.min(100, 200)); // 최소값 반환: .mix(데이터1, 데이터2)
        ...
        String message = "Marry Christmas!";

        System.out.println(message.length()); // 9 - 문자뿐만 아니라 공백, 기호까지 포함

        boolean isEquals = message.equals("Marry Christmas^^");
        System.out.println(isEquals); // false

        // substring(int start, int end)
        // : 시작 인덱스(포함)부터 끝 인덱스(미포함)까지의 부분 문자열을 반환
        // - 인덱스 번호는 0부터 시작, 공백과 기호까지 포함
        System.out.println(message.substring(3, 8)); // 크리스마스

        System.out.println(message); // 메리 크리스마스! - 문자열은 불변(immutable) 데이터
