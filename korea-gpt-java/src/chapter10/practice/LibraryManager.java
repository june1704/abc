    ...
    public List<Item> search(String keyword) {
        ...
        for (Item item : items) {
            if (item.getName().contains(keyword) // contains: 포함하다
                //||/*또는*/ (item instanceof Book/*item은 book에 속해 있냐: ㅇㅇ*/&&/*그렇다면*/ ((Book)item)/*다운캐스팅해서*/.getAuthor().contains(keyword)/*저자를 검색 할거다*/
                //                                                                 ||/*또는*/ ((Book)item)/*다운캐스팅해서*/.getPublisher().contains(keyword))/*출판사를 검색 할거다*/
                || (item instanceof Book && ((Book)item).getAuthor().contains(keyword))
                || (item instanceof Book && ((Book)item).getPublisher().contains(keyword))) {
                    foundItems.add(item);
                }
            }
...
//        for (Item item : items) {
//            int price = ((Book) item).getPrice();
//            if (item instanceof Book && ((Book) item).getPrice() >= minPrice && ((Book) item).getPrice() <= maxPrice) {
//                result.add(item);
//            }
//        }
        return result;
